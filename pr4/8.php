...
echo "<ol>";
for ($j = 0; $j < count($track); $j++) {
    echo "<li>({$track[$j]['id_track']}) {$track[$j]['name']} (id альбома: {$track[$j]['id_album']})</li>";
}
echo "</ol>";

// --- 3. Цикл foreach (таблица) ---
echo "<h3>3. Цикл foreach (табличное представление)</h3>";
...
// --- 4. Цикл while (таблица) ---
echo "<h3>4. Цикл while (табличное представление)</h3>";
echo "<table border='1' style='border-collapse: collapse; width: 100%;'>";
echo "<tr><th>id</th><th>Трек</th><th>Примечание</th><th>Id-альбома</th></tr>";
$k = 0;
while ($k < count($track)) {
